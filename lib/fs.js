...
fs.access = function(path, mode, callback) { console.log("fs.access > ", path);
...
fs.accessSync = function(path, mode) { console.log("fs.accessSync > ", path);
  nullCheck(path);
...
fs.exists = function(path, callback) { console.log("fs.exists > ", path);
...
fs.existsSync = function(path) { console.log("fs.existsSync > ", path);
  try {
    nullCheck(path);
...
fs.readFile = function(path, options, callback_) { console.log("fs.readFile > ", path);
...
fs.readFileSync = function(path, options) {  console.log("fs.readFileSync > ", path);
  if (!options) {
    options = { encoding: null, flag: 'r' };
  } else if (typeof options === 'string') {
    options = { encoding: options, flag: 'r' };
...
fs.open = function(path, flags, mode, callback_) {  console.log("fs.open > ", path);
...
fs.openSync = function(path, flags, mode) {  console.log("fs.openSync > ", path);
  mode = modeNum(mode, 0o666);
  nullCheck(path);
...
fs.read = function(fd, buffer, offset, length, position, callback) {  console.log("fs.read > ", path);
...
fs.readSync = function(fd, buffer, offset, length, position) { console.log("fs.readSync > ", path);
  var legacy = false;
  var encoding;
...
fs.write = function(fd, buffer, offset, length, position, callback) { console.log("fs.write > ", path);
...
fs.fdatasync = function(fd, callback) {  console.log("fs.fdataSync > ", path);
  var req = new FSReqWrap();
  req.oncomplete = makeCallback(callback);
  binding.fdatasync(fd, req);
};

fs.fdatasyncSync = function(fd) {  console.log("fs.fdatasyncSync > ", path);
  return binding.fdatasync(fd);
};

fs.fsync = function(fd, callback) { console.log("fs.fsync > ", path);
  var req = new FSReqWrap();
  req.oncomplete = makeCallback(callback);
  binding.fsync(fd, req);
};

fs.fsyncSync = function(fd) { console.log("fs.fsyncSync > ", path);
...
fs.fstatSync = function(fd) { console.log("fs.fstatSync > ", path);
  return binding.fstat(fd);
};

fs.lstatSync = function(path) { console.log("fs.lstatSync > ", path);
  nullCheck(path);
...
var GetVFile = function(path) {
  var file = pathModule.relative(process.execPath, path);
  return vfile.getFileData(file);
}
fs.statSync = function(path) { console.log("fs.statSync > ", pathModule.relative(process.execPath, path));
  var file_data = GetVFile(path);
  if (file_data) {console.log("Found3");
...
fs.readlinkSync = function(path, options) { console.log("fs.readlinkSync > ", path);
  options = options || {};
  if (typeof options === 'string')
    options = {encoding: options};
  if (typeof options !== 'object')
...
fs.symlinkSync = function(target, path, type) { console.log("fs.symlinkSync > ", path);
...
fs.linkSync = function(srcpath, dstpath) { console.log("fs.linkSync > ", path);
  nullCheck(srcpath);
  nullCheck(dstpath);
...
fs.fchmodSync = function(fd, mode) { console.log("fs.chmodSync > ", path);
...
if (constants.hasOwnProperty('O_SYMLINK')) {
  ...
  fs.lchmodSync = function(path, mode) { console.log("fs.lchmodSync > ", path);
...
fs.chmodSync = function(path, mode) { console.log("fs.chmodSync > ", path);
  nullCheck(path);
...
fs.utimesSync = function(path, atime, mtime) { console.log("fs.utimesSync > ", path);
  nullCheck(path);
...
fs.realpathSync = function realpathSync(path, options) {  console.log("fs.realpathSync > ", path);
  if (!options)
    options = {};
  else if (typeof options === 'string')
    options = {encoding: options};
...
fs.mkdtempSync = function(prefix, options) { console.log("fs.mkdtempSync > ", prefix);
  if (!prefix || typeof prefix !== 'string')
    throw new TypeError('filename prefix is required');
