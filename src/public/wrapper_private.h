...
#ifndef SRC_WRAPPER_PRIVATE_H
#define SRC_WRAPPER_PRIVATE_H
...
#define JS_OBJECT_FROM_PERSISTENT__(x) JS_HANDLE_VALUE::New(__contextORisolate, x)
...
#define STRING_TO_STD(x) *node::Utf8Value(__contextORisolate, x)
...
#define BUFFER__DATA(x) (static_cast<char*>((x).As<v8::Uint8Array>()->Buffer()->GetContents().Data()) + ((x).As<v8::Uint8Array>()->ByteOffset()))
...
class JS_Method {
 public:
  ...
  JS_Method() : native_method_(0), is_native_method_(false), interface_id_(-1) {}
};
...
class JSFunctionWrapper {
 ...
 public:
  ...
  static JS_WEAK_CALLBACK_METHOD(v8::Object, void,
...
#define UNWRAP_RESULT(x) \
  JSValueWrapper *wrap = (JSValueWrapper *)x

#endif // SRC_WRAPPER_PRIVATE_H
